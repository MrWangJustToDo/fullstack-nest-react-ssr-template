...
export const compilerPromise = (
  ...
  if (development && dynamicRouter) {
    logger().info(
      `[${name}] Dynamic Router enabled!, now you can create a new page under the pages folder for fast refresh`,
    );
  }
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    compiler.hooks.done.tap(name, (stats) => {
      if (!stats.hasErrors()) {
        development
          ? logger().info(
              `[${name}] compiler done, compiler count: ${count++} -- time: ${
                stats.endTime - stats.startTime
              } ms`,
            )
          : logger().info(
              `[${name}] production code compiler done -- time: ${
                stats.endTime - stats.startTime
              } ms`,
            );
        if (name === 'server') {
