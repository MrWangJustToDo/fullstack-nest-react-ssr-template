...
export class DynamicRouter {
  ...
  public getRouterConfig = (prePath: string, dirName: string) => {
    return new Promise<Array<{ path: string; componentPath: string }>>(
      (resolve) => {
...
export const dynamicRouteConfig: DynamicRouteConfig[] = ${routerResult};`;
  ...
  getDynamicRouter = async () => {
    const pages = path.resolve(process.cwd(), 'src', 'app', 'pages');
    const dynamicRouteFilename = path.resolve(
      process.cwd(),
      'src',
      'app',
      'router',
    ...
    if (!cacheCheck) {
      ...
      if (currentTemplate === newTemplate) {
        logger().info(
          `[${this.side}] file router do not need update from template`,
        );
        this.cache.value = routerResult;
      } else {
