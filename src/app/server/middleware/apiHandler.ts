...
const excludeApiMiddlewareHandler: MiddlewareFunction = async (
  ctx,
  nextMiddleware,
) => {
  const { req, next } = ctx;
...
const catchMiddlewareHandler: MiddlewareFunction = async (
  ctx,
  nextMiddleware,
) => {
  ...
  try {
    ...
    serverLog(
      `url: ${req.originalUrl}, method: ${req.method} error, ${
        (e as Error).message
      }`,
      'error',
    );
    ...
    if (errorHandler && typeof errorHandler === 'function') {
      ...
      res
        .status(e instanceof RenderError ? e.code : 500)
        .json({ data: (e as Error).toString() });
    }
  }
};
...
export const compose = (...middleWares: MiddlewareFunction[]) => {
  return function (
    ...
    next: MiddlewareFunction | RequestHandlerType,
  ) {
    let runTime = 0;
    let index = -1;
    // 需要加上死循环判断
    function dispatch(i: number): Promise<void> {
      if (i <= index) {
        ...
        throw new RenderError(
          'compose index error, every middleware only allow call once',
          500,
        );
      }
      // 防止中间件死循环
      ...
      if (runTime > middleWares.length + 5) {
        throw new RenderError(
          'call middleWare many times, look like a infinite loop and will stop call next',
          500,
        );
      }
      index = i;
...
export const wrapperMiddlewareRequest = function (
  config: MiddlewareConfig,
  composed: ReturnType<typeof compose> = composedMiddleware,
) {
