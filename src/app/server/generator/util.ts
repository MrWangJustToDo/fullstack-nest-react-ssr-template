...
export const getStaticPageOutputPath = (fileName: string) =>
  path.resolve(process.cwd(), 'dist', 'app', 'pages', fileName);
...
export const getAllStaticRouters = (allRouters: Record<string, boolean>) =>
  Object.keys(allRouters)
    ...
    .map((p) => ({
      url: `http://${process.env.PROD_HOST}:${process.env.PROD_PORT}${p.slice(
        1,
      )}`,
      p,
    }));
...
export const generateStaticPage = (pathConfig: {
  ...
  pathConfig: { url: string; p: string };
  rawData?: string;
  error?: Error;
}> =>
  new Promise((r) => {
...
export const prepareOutputPath = (p: string) =>
  fs.mkdir(path.dirname(p), { recursive: true }).catch();

export const writeContentToFilePath = (p: string, content: string) =>
