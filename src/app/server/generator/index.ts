...
export const generateStaticPage = async () => {
  ...
  const allStaticRoutersPage = await Promise.all(
    allStaticRouters.map((pathConfig) => _generateStaticPage(pathConfig)),
  );
  const allStaticRoutersPageWithFilePath = allStaticRoutersPage
    ...
    .map((config) => ({
      ...
      pathConfig: {
        ...config.pathConfig,
        ...getFileNameFromPath(config.pathConfig),
      },
    }))
    .map((config) => ({
      ...config,
  ...
  const generateManifest = await Promise.all(
    allStaticRoutersPageWithFilePath.map((config) =>
      prepareOutputPath(config.pathConfig.filePath).then(() =>
        writeContentToFilePath(
          config.pathConfig.filePath,
          config.rawData as string,
        )
  ...
  await writeContentToFilePath(
    ...
    JSON.stringify(
      generateManifest
        ...
        .map((config) => ({
          [config.config.pathConfig.p]: config.config.pathConfig.filePath,
        }))
        .reduce((p, c) => ({ ...p, ...c }), {}),
    ),
  );
