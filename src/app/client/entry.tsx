...
const store = createUniversalStore({
  preloadedState: JSON.parse(
    preLoadStateElement?.innerHTML || '{}',
  ) as RootState,
});

window.__ENV__ = JSON.parse(preLoadEnvElement?.innerHTML || '{}');

window.__PRELOAD_STORE_STATE__ = JSON.parse(
  preLoadStateElement?.innerHTML || '{}',
);
...
safeData(
  ...
  '__PRELOAD_STORE_STATE__',
);

if (window.__ENV__.isPURE_CSR) {
  const { loadCurrentLang } = require('@app/util/i18n');
  const root = createRoot(place);
  loadCurrentLang(store.dispatch, window.__ENV__.LANG).then(() =>
    root.render(<Root store={store} />),
  );
} else {
  if (
    !window.__ENV__.isSSR ||
    (window.__ENV__.isDEVELOPMENT && window.__ENV__.isMIDDLEWARE)
  ) {
    const root = createRoot(place);
    root.render(<Root store={store} />);
  } else {
