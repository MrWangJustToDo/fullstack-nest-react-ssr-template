...
// TODO
const outputPath = (env: "server" | "client", isDEV: boolean) => path.resolve(process.cwd(), isDEV ? "dev" : "dist", "ui", env);
...
export const outputConfig = ({ env, isDEV, isMIDDLEWARE, DEV_HOST, WDS_PORT, PROD_HOST, PROD_PORT }: SafeGenerateActionProps): Configuration["output"] =>
  env === "client"
    ? {
        ...
        // 引入资源的url路径
        publicPath: isDEV ? (isMIDDLEWARE ? "/dev/" : `http://${DEV_HOST}:${WDS_PORT}/dev/`) : `http://${PROD_HOST}:${PROD_PORT}/ui/client/`,
      }
    : {
        clean: true,
        path: outputPath(env, Boolean(isDEV)),
